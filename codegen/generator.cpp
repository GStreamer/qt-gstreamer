...
int main(int argc, char *argv[])
{
    ...
    QTextStream(stdout) << "// Autogenerated by the QtGstreamer helper code generator" << endl
                        << "#include <gst/gst.h>" << endl
                        << "#include <boost/static_assert.hpp>" << endl << endl;

    for (int i=1; i<argc; ++i) {
        QString fileName(QFile::decodeName(argv[i]));
        if (fileName.endsWith(".h") && QFile::exists(fileName)) {
